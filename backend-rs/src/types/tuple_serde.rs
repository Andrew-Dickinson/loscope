...
mod tests {
    ...
    fn serialize_tuple2_negative_values() {
        assert_eq!(serde_json::to_string(&W2((-3.5, 0.0))).unwrap(), "[-3.5,0.0]");
    }
    ...
    fn serialize_tuple3_produces_json_array() {
        assert_eq!(serde_json::to_string(&W3((1.0, 2.0, 3.0))).unwrap(), "[1.0,2.0,3.0]");
    }

    #[test]
    fn serialize_tuple3_negative_values() {
        assert_eq!(serde_json::to_string(&W3((-1.0, 0.0, 999.5))).unwrap(), "[-1.0,0.0,999.5]");
    }
