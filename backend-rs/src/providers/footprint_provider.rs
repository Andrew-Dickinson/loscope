...
impl FootprintProvider for StringBackedFootprintProvider {
    async fn get_footprint(&self, bin_id: BINId) -> Result<Polygon, AssetErr> {
        ...
        let wkt_string = self.string_provider.get_string(
            AssetType::BuildingFootprintWKT,
            bin_id.as_str(),
        ).await?;

        Polygon::try_from_wkt_str(&wkt_string)
            .or_else(|err| Err(AssetErr::AssetContentError(
                format!("Invalid WKT string found in database for {bin_id:?}: {err}")
            )))
    }
}
...
mod tests {
    ...
    impl MockStringProvider {
        fn ok(wkt: &str) -> (Self, Arc<Mutex<Captured>>) {
            let captured = Arc::new(Mutex::new(Captured { asset_type: None, identifier: None }));
        ...
        fn err(e: AssetErr) -> Self {
            MockStringProvider {
                result: Err(e),
                captured: Arc::new(Mutex::new(Captured { asset_type: None, identifier: None })),
            }
        }
    }
    ...
    impl StringProvider for MockStringProvider {
        async fn get_string(&self, asset_type: AssetType, identifier: &str) -> Result<String, AssetErr> {
            ...
            match &self.result {
                Ok(s) => Ok(s.clone()),
                Err(AssetErr::AssetDownloadError(msg)) => Err(AssetErr::AssetDownloadError(msg.clone())),
                Err(AssetErr::AssetNotFound(msg)) => Err(AssetErr::AssetNotFound(msg.clone())),
                Err(AssetErr::AssetContentError(msg)) => Err(AssetErr::AssetContentError(msg.clone())),
                Err(AssetErr::LocalFileSystemError(msg)) => Err(AssetErr::LocalFileSystemError(msg.clone())),
                Err(AssetErr::UnsupportedAssetType(msg)) => Err(AssetErr::UnsupportedAssetType(msg.clone())),
            }
        }
    }
    ...
    async fn passes_correct_asset_type_and_bin_id() {
        ...
        make_provider(mock).get_footprint(valid_bin()).await.unwrap();
    ...
    async fn propagates_download_error_from_string_provider() {
        let provider = make_provider(MockStringProvider::err(
            AssetErr::AssetDownloadError("db error".to_string()),
        ));
    ...
    async fn propagates_not_found_error_from_string_provider() {
        let provider = make_provider(MockStringProvider::err(
            AssetErr::AssetNotFound("not found".to_string()),
        ));
        let result = provider.get_footprint(valid_bin()).await;
