        ...
        let inner = move || -> Result<Array2::<u16>, Box<dyn std::error::Error>> {
            ...
            if height != subgrid_tile_side_length_usft_usize || width != subgrid_tile_side_length_usft_usize {
                return Err(Box::new(io::Error::new(
                    io::ErrorKind::InvalidData, format!("Invalid tiff file size: ({width}, {height})")
                )))
            }
        ...
        let image_data = inner()
            .or_else(|err| Err(
                AssetErr::AssetContentError(
                    format!("Error parsing tile tiff for id {id}: {err}")
                )
            ))?;
...
impl ElevationTileProvider for CachingElevationTileProvider {
    async fn get_elevation_tile(&self, tile_id: TileId) -> Result<ElevationTile, AssetErr> {
        ...
        let asset_res = self.asset_provider.get_asset(AssetType::ElevationTile, asset_id).await;

        match asset_res {
            Ok(asset) => {
                ElevationTile::read_from_tiff(tile_id, asset)
            },
            Err(err) => {
                if let AssetErr::AssetNotFound(_) = err {
                    ...
                    File::create(path.as_path()).map_err(|err| err.to_string())
                        ...
                        .or_else(|err| {
                            Err(AssetErr::LocalFileSystemError(format!("Error writing empty tile to {path:?}: {err}")))
                        })?;
                    Ok(tile)
                } else {
                    Err(err)
...
mod tests {
    ...
    fn sample_tif_path() -> std::path::PathBuf {
        std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("tests/resources/2235_13.tif")
    }

    fn write_gray16_tiff(file: &mut File, width: u32, height: u32, data: &[u16]) {
        TiffEncoder::new(file).unwrap()
            .write_image::<colortype::Gray16>(width, height, data).unwrap();
    }

    fn write_gray8_tiff(file: &mut File, width: u32, height: u32, data: &[u8]) {
        TiffEncoder::new(file).unwrap()
            .write_image::<colortype::Gray8>(width, height, data).unwrap();
    }
    ...
    fn read_from_tiff_wrong_colortype_fails() {
        ...
        write_gray8_tiff(&mut File::create(&path).unwrap(), side, side, &vec![0u8; (side * side) as usize]);
    ...
    fn write_to_tiff_roundtrip_sample_file() {
        ...
        let restored = ElevationTile::read_from_tiff(
    ...
    impl MockAssetProvider {
        fn returning_file(path: std::path::PathBuf) -> Self {
            Self { get_asset_result: Ok(path.clone()), local_asset_path: path }
        }
        ...
        fn returning_err(err: AssetErr) -> Self {
            Self { get_asset_result: Err(err), local_asset_path: std::path::PathBuf::new() }
        }
    }
