...
struct Adler32 { s1: u64, s2: u64 }

impl Adler32 {
    fn new() -> Self { Self { s1: 1, s2: 0 } }
    ...
    fn finish(self) -> u32 { ((self.s2 << 16) | self.s1) as u32 }
}
...
struct BitWriter { buf: Vec<u8>, acc: u32, nbits: u8 }
...
fn emit_lit(w: &mut BitWriter, byte: u8) {
    let s = byte as u32;
    if s <= 143 { w.msb(48 + s, 8); } else { w.msb(400 + (s - 144), 9); }
}

#[inline]
fn emit_eob(w: &mut BitWriter) { w.msb(0, 7); }

fn emit_backref(w: &mut BitWriter, length: usize, distance: usize) {
    let (ls, le, lb) = length_code(length);
    if ls <= 279 { w.msb(1 + (ls - 257), 7); } else { w.msb(192 + (ls - 280), 8); }
    if lb > 0 { w.lsb(le, lb); }
...
fn emit_backref_run(w: &mut BitWriter, mut total: usize, dist: usize) {
    ...
    for _ in 0..total { emit_lit(w, 0); } // tail < 3 (only reachable when total was < 3)
}
...
fn emit_zero_run(w: &mut BitWriter, total: usize) {
    if total == 0 { return; }
    if total < 6 { for _ in 0..total { emit_lit(w, 0); } return; }
    for _ in 0..3 { emit_lit(w, 0); }
...
fn length_code(len: usize) -> (u32, u32, u8) {
    match len {
        ...
        11..=12  => (265, (len-11) as u32, 1), 13..=14  => (266, (len-13) as u32, 1),
        15..=16  => (267, (len-15) as u32, 1), 17..=18  => (268, (len-17) as u32, 1),
        19..=22  => (269, (len-19) as u32, 2), 23..=26  => (270, (len-23) as u32, 2),
        27..=30  => (271, (len-27) as u32, 2), 31..=34  => (272, (len-31) as u32, 2),
        35..=42  => (273, (len-35) as u32, 3), 43..=50  => (274, (len-43) as u32, 3),
        51..=58  => (275, (len-51) as u32, 3), 59..=66  => (276, (len-59) as u32, 3),
        67..=82  => (277, (len-67) as u32, 4), 83..=98  => (278, (len-83) as u32, 4),
        99..=114 => (279, (len-99) as u32, 4), 115..=130 => (280, (len-115) as u32, 4),
        131..=162 => (281, (len-131) as u32, 5), 163..=194 => (282, (len-163) as u32, 5),
        195..=226 => (283, (len-195) as u32, 5), 227..=257 => (284, (len-227) as u32, 5),
        258 => (285, 0, 0),
...
fn dist_code(dist: usize) -> (u32, u32, u8) {
    match dist {
        ...
        5..=6     => ( 4, (dist- 5) as u32, 1), 7..=8     => ( 5, (dist- 7) as u32, 1),
        9..=12    => ( 6, (dist- 9) as u32, 2), 13..=16   => ( 7, (dist-13) as u32, 2),
        17..=24   => ( 8, (dist-17) as u32, 3), 25..=32   => ( 9, (dist-25) as u32, 3),
        33..=48   => (10, (dist-33) as u32, 4), 49..=64   => (11, (dist-49) as u32, 4),
        65..=96   => (12, (dist-65) as u32, 5), 97..=128  => (13, (dist-97) as u32, 5),
        129..=192  => (14, (dist-129) as u32, 6), 193..=256  => (15, (dist-193) as u32, 6),
        257..=384  => (16, (dist-257) as u32, 7), 385..=512  => (17, (dist-385) as u32, 7),
        513..=768  => (18, (dist-513) as u32, 8), 769..=1024 => (19, (dist-769) as u32, 8),
        1025..=1536  => (20, (dist-1025) as u32, 9), 1537..=2048  => (21, (dist-1537) as u32, 9),
        2049..=3072  => (22, (dist-2049) as u32, 10), 3073..=4096  => (23, (dist-3073) as u32, 10),
        4097..=6144  => (24, (dist-4097) as u32, 11), 6145..=8192  => (25, (dist-6145) as u32, 11),
        8193..=12288  => (26, (dist-8193) as u32, 12), 12289..=16384 => (27, (dist-12289) as u32, 12),
        16385..=24576 => (28, (dist-16385) as u32, 13), 24577..=32768 => (29, (dist-24577) as u32, 13),
...
fn pixel_run(color: [u8; 4]) -> [u8; 32] {
    let mut out = [0u8; 32];
    for i in 0..UPSCALE { out[i*4..i*4+4].copy_from_slice(&color); }
    out
}
...
fn precompute_row_meta(source_filled: &[bool], cell_color: &[[u8; 4]]) -> Vec<RowMeta> {
    (0..TILE_SIDE).map(|sy| {
        ...
        if !has_fill {
            return RowMeta { has_fill: false, sum_b: 0, wsum_b: 0 };
        }
        ...
        for sx in 0..TILE_SIDE {
            let color = if source_filled[sy * TILE_SIDE + sx] { cell_color[sy * TILE_SIDE + sx] } else { [0u8; 4] };
        ...
        RowMeta { has_fill: true, sum_b: sum_b % ADLER_MOD, wsum_b: wsum_b % ADLER_MOD }
    }).collect()
}
...
mod tests {
    ...
    fn empty_image_is_fully_transparent() {
        ...
        for y in 0..OUT_SIDE as u32 {
            for x in 0..OUT_SIDE as u32 {
                assert_eq!(img.get_pixel(x, y).0[3], 0, "pixel ({x},{y}) should be transparent");
            }
        }
    }
    ...
    fn single_cell_color_and_position() {
        ...
        assert_eq!(img.get_pixel(0, row_start - 1).0[3], 0, "above cell should be transparent");
        assert_eq!(img.get_pixel(UPSCALE as u32, row_start).0[3], 0, "right of cell should be transparent");
    }
