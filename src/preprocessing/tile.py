...
from .tile_id import GRID_N, TILE_SIDE_USFT, make_tile_id, tile_nw_corner


@dataclass
class TileData:
    tile_id: str
    x_offset: int   # NW corner X (NYS usft)
    y_offset: int   # NW corner Y (NYS usft)
...
def split_tiles(height_grid, file_id, origin):
    ...
    for xi in range(GRID_N):
        for yi in range(GRID_N):
            ...
            x_off, y_off = tile_nw_corner(origin, xi, yi)
